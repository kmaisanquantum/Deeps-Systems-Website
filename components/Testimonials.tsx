...
const Testimonials: React.FC = () => {
  ...
  return (
    <section id="testimonials" className="py-24 relative overflow-hidden bg-slate-900/10">
      ...
      <div className="absolute top-0 right-0 w-64 h-64 bg-green-500/5 rounded-full blur-[100px] -z-10"></div>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((t, i) => (
            <div 
              key={i} 
              className="glass p-8 rounded-3xl border border-white/5 transition-all duration-500 hover:border-green-500/40 hover:-translate-y-2 group flex flex-col justify-between shadow-xl hover:shadow-[0_22px_40px_-15px_rgba(34,197,94,0.2)] cursor-default"
            >
              <div>
                <div className="mb-6 p-3 rounded-xl bg-green-500/10 inline-block group-hover:scale-110 transition-transform duration-300">
                  <Quote className="w-6 h-6 text-green-400 fill-green-400/20" />
                </div>
              ...
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full quantum-gradient flex items-center justify-center text-white font-bold text-xl shadow-lg shadow-green-500/20">
                  {t.author.charAt(0)}
                </div>
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium uppercase tracking-widest">
                    {t.position} @ <span className="text-green-400/80">{t.company}</span>
                  </p>
                </div>
              </div>
            </div>
